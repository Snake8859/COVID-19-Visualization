...
<div id="BottomBar" class="bottomBar opacity0">
    <div class="bar-content-bottom" id="bottomContent">
            ...
            <div class="bottom-right" id="bottomRightPart">
                <ul @click="showRumorBody">
                    <li v-for="(rumor, index) in rumor_list">
                        <div class="bottom-item" :data-index="index">
                            <div :data-index="index">
                                <img src="images/icons/rumor.png" :data-index="index">
                            </div>

                            <div class="bottom-item-info" :data-index="index">
                                <p :data-index="index">
                                    {{rumor.title}}
                                </p>
                                <p :data-index="index">
    ...
    resolve({
      ...
      methods: {
        ...
        _realTimeNews: function () {
          ...
          axios
            .get(configOptions.MyService.newsData) //【自行数据接口】
            .then(response => {
              // handle success
              ...
              this.$nextTick(() => {
                //确保li内数据渲染完成，再调用vTicker。否则获取不到li节点内容
                // 在这里面去获取DOM
        ...
        _realTimeRumor: function () {
          ...
          axios
            .get(configOptions.MyService.rumorData) //【自行数据接口】
            .then(response => {
              // console.log('==底部范围==')
        ...
        // },
        //显示谣言主体内容
        showRumorBody: function (e) {
          const nodeName = e.target.nodeName.toLocaleLowerCase();
          const i = Number(e.target.dataset.index);
          if (nodeName === 'p' || nodeName === 'img' || nodeName === 'div') {
            alert(this.rumor_list[i].body);
          }
        },
        //时间戳转时间
        _transformTime: function (timestamp) {
          if (timestamp != undefined) {
            ...
            return y + '-' + this._addZero(M) + '-' + this._addZero(d) + ' ' + this._addZero(h) + ':' + this._addZero(m) + ':' + this._addZero(s);
          } else {
            return '';
          }
        },
