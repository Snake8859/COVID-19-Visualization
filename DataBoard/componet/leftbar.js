        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="input-group">
                    <input type="text" class="form-control" v-model="searchValue" @keyup.enter="moveToCountryBySearch">
    ...
    resolve({
      data: function () {
        return {
          ...
          clickLeftCount: 0,
          //渲染色带
          colorCard: ['#F8E4CC', '#CF6C73', '#B65558', '#B5385D', '#A7273D'],
          searchValue: ''
        };
      },
      ...
      methods: {
        ...
        _initLeftListData: function () {
          ...
          axios
            .get(configOptions.MyService.worldAreaData) //【自行数据接口】
            .then(response => {
              // console.log('===左侧访问===');
              ...
              for (let index in this.worldDataCache) {
                ...
                if (item.cities == null) {
                  ...
                  let flag; //状态标识
                  if (currentConfirmedCount >= 0 && currentConfirmedCount <= 500) {
                    //确诊人数0~500之间，状态存在隐患
                    flag = 0;
                  } else if (currentConfirmedCount > 500 && currentConfirmedCount < 50000) {
                    //确诊人数500~50000之间，状态一般
                    flag = 1;
                  } else if (currentConfirmedCount >= 50000) {
                    //确诊人数大于50000
                    flag = 2;
                  }
        ...
        moveToCountry: function (countryEnglishName, countryName, count) {
          ...
          for (let index in worldFeatures) {
            //遍历世界要素数据
            feature = worldFeatures[index];
            let name = feature.properties.name;
            if (name == countryEnglishName) {
              //若为所点击的国家列表要素数据
        ...
        _generateLeftCurve: function (countryEnglishName = 'globalStatistics', countryName = '世界') {
          axios
            .get(configOptions.geojson.countryFolder + countryEnglishName + '.json')
            .then(response => {
              // console.log(response);
          ...
          if (status) {
            ...
            map.on('mousemove', 'world', e => {
              ...
              if (count != undefined) {
                ...
                popup
                  ...
                  .setHTML(
                    "<div><strong style='color:red'>" +
                      hoveredLayer.properties.countryCN +
                      '</strong>' +
                ...
                popup.setLngLat([e.lngLat.lng, e.lngLat.lat]).setHTML("<div><strong style='color:black'>未知</strong></div>").addTo(map);
              }
            });
