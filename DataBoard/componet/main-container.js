...
Vue.component('main-container', {
    ...
    methods: {
        ...
        _loadGeoJsonData: function() {
            //加载世界国家geojson数据
            axios.get(configOptions.geojson.world)
                .then((response) => {
            ...
            //加载中国各省份geojson数据
            axios.get(configOptions.geojson.china)
                .then((response) => {
                    // console.log(response);
        ...
        _loadOverAllData: function() {
            //加载世界和中国疫情概况数据 
            axios.get(configOptions.MyService.overAllData) //【自行数据接口】
        ...
        _zoomLevelControl: function() {
            this.map.on('zoomend', (e) => {
                ...
                if (zoomLevel <= 2 && this.countryName != '世界') { //若zoomLevel小于等于2，则需要调用子组件重新初始化世界概况
                    //调用子组件方法，重新初始化世界疫情概况
                    this.$children[0]._initLeftOverAllData();
                    this.$children[0]._generateLeftCurve();
                }
                if (zoomLevel <= 3 && this.provinceName != '中国') { //若zoomLevel小于等于3，则需要调用子组件重新初始化中国概况
                    //调用子组件方法，重新初始化中国疫情概况
        ...
        _loadPeopleAnalysis: function(status) {
            ...
            if (status) { //若选中
                ...
                //加载岳麓山边界
                axios.get(configOptions.geojson.yls)
                    .then((res) => {
                        // console.log(res);
                        map.addLayer({
                            'id': 'yls',
                ...
                this.heatFlag = false;
                if (map.getLayer('heatLayer')) { //若是已有图层，则删除
                    map.removeLayer('heatLayer');
                    map.removeSource('heatLayer');
                }
                if (map.getLayer('yls')) { //若是已有图层，则删除
                    map.removeLayer('yls');
        ...
        _loadSubwayAnalysis: function(status) {
            ...
            if (status) { //若选中
                ...
                step = new SetStep({
                    content: '#subway_bar',
                    showBtn: false,
                    steps: ['07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00',
                ...
                //加载地铁线要素
                axios.get(configOptions.geojson.hz)
                    .then((res) => {
                        // console.log(res);
                ...
                if (map.getLayer('lineLayer')) { //若是已有图层，则删除
        ...
        _peopleAnalysisByHour: function() {
            ...
            if (datetime != "") {
                ...
                axios.get(configOptions.people.dataUrl, {
                        params: {
                            dateTime: datetime,
                            hour: hour
                        }
        ...
        _peopleAnalysisByDay: function() {
            ...
            if (date != "") {
                axios.get(configOptions.people.allDataUrl, {
                        params: {
                            date: date
                        }
                    })
        ...
        _subwayAnalysis: function() {
            ...
            axios.get(configOptions.subway.dataUrl)
                .then((res) => {
                    // console.log(res);
                    //获得时间的客流量数据
        ...
        _generateLineMap: function(lineGeoJson, colorList, hour) {
            ...
            let map = this.map;
            if (map.getLayer('lineLayer')) { //若是已有图层，则移除事件和图层
        ...
        _hideLoader2: function() { //隐藏加载logo2
