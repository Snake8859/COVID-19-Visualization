        ...
        resolve({
            ...
            methods: {
                ...
                _initRightChart: function() {
                    //清除原有数据
                    this.regional_bar_option.yAxis.data.length = 0;
                    ...
                    axios.get(configOptions.MyService.chinaAreaData) //【自行数据接口】
                        .then((response) => {
                            ...
                            for (let index in this.chinaDataCache) {
                                ...
                                if (item.cities != null) { //提取中国各省份的疫情信息
                                    ...
                                    if (currentConfirmedCount != 0 && suspectedCount >= 0) { //若确诊人数不为0，且疑似病例大于等于0，显示在柱状图中
                                        this.regional_bar_option.yAxis.data.push(provinceName);
                            ...
                            this.$nextTick(() => { //确保内数据渲染完成，再调用滚动条设置和初始化底部数据列表
                                //配置图表1
                ...
                _initRightListData: function() {
                    for (let index in this.chinaDataCache) { //遍历中国疫情现状缓存
                        ...
                        if (item.cities != null) { //提取中国各省份的疫情信息
                            ...
                            let currentConfirmedCount = item.currentConfirmedCount; //省份当前确诊
                            if (currentConfirmedCount > 0 && currentConfirmedCount < 15) { //确诊人数在0~15之间，状态为一般
                                flag = 1;
                            } else if (currentConfirmedCount >= 15) { //确诊人数大于15，状态为严重
                                flag = 2;
                            }
                ...
                _moveToProvince: function(provinceEnglishName, provinceName, count) {
                    ...
                    for (let index in chinaFeatures) { //遍历中国要素数据
                        ...
                        if (name == provinceName) { //若为所点击省份的名称
                            // console.log(feature);
                ...
                _generateProvinceBar: function(provinceEnglishName, provinceName) {
                    ...
                    for (let index in this.chinaDataCache) {
                        ...
                        if (item.provinceEnglishName == provinceEnglishName) {
                            ...
                            cities = item.cities;
                            // console.log(cities);
                            if (provinceEnglishName == 'Beijing') { //若为北京市，提示北京市数据存在问题
                                alert(item.comment);
                            } else if (cities.length != 0) {
                                for (let id in cities) {
                                    let city = cities[id];
                                    this.regional_bar_option.yAxis.data.push(city.cityName);
                                ...
                                this.regional_bar_option.yAxis.data.push(item.provinceName);
                ...
                loadChinaColorLayer: function(status) {
                    ...
                    if (status) { //选中加载
                        ...
                        map.on('mousemove', 'province', (e) => {
                            ...
                            let status = '安全';
                            let color = 'green';
                            if (count == 0) { //确诊人数为0，状态为安全
                                status = '安全';
                                color = 'green';
                            } else if (count > 0 && count < 15) { //确诊人数在0~15之间，状态为一般
                                status = '一般';
                                color = 'orange';
                            } else if (count >= 15) { //确诊人数大于15，状态为严重
                                status = '严重';
                                color = 'red';
                            }
                            map.getCanvas().style.cursor = 'pointer';
                            popup.setLngLat([e.lngLat.lng, e.lngLat.lat])
                                .setHTML("<div><strong style='color:red'>" + hoveredLayer.properties.name + "</strong>" +
