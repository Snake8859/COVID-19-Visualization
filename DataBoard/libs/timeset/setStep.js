...
SetStep.prototype.init = function (arg) {
  ...
  if (this.opt.clickAble) {
    stepsHtml.find('li').on('click', function () {
      ...
      //间隔1.5秒后，等动画结束后执行
      // app.peopleAnalysis();
      if (vm.$children[1].heatFlag) {
        // console.log("执行岳麓山人流量分析");
        vm.$children[1]._peopleAnalysisByHour();
        // vm.$children[1]._generateHeatMapByDay();
      }
      if (vm.$children[1].subwayFlag) {
        // console.log("执行杭州地铁客流量分析");
